...
def main():
    ...
    # --- lxplus: HTCondor submission / monitoring (parallel alternative to CRAB) -----
    parser.add_argument('--submitCondorJobs', action='store_true',
                       help='[3alt][lxplus][condor] Submit pre-selection jobs to HTCondor based on the DAS '
                            'dataset JSON generated by --generateDASDatasetJSON. Parallel alternative to '
                            '--submitPreSelectionJobs -- writes output directly into the final flat '
                            'preselection layout (skips --generateCrabDatasetJSON/--consolidateCrabOutput, '
                            'which are CRAB-only). Uses scripts/condor/submit_preselection_condor.py. '
                            'Requires `module load lxbatch/eossubmit` (all job files must be on EOS).')
    parser.add_argument('--checkCondorStatus', action='store_true',
                       help='[4alt][lxplus][condor] Check HTCondor job status for jobs submitted with '
                            '--submitCondorJobs. Uses scripts/condor/checkCondorStatus.py.')
    parser.add_argument('--resubmitHeldCondorJobs', action='store_true',
                       help='[4alt][lxplus][condor] With --checkCondorStatus: condor_release held jobs.')
    parser.add_argument('--resubmitMissingCondorJobs', action='store_true',
                       help='[4alt][lxplus][condor] With --checkCondorStatus: resubmit jobs whose output is '
                            'missing and not currently queued.')
    parser.add_argument('--condorFilesPerJob', type=int, default=1,
                       help='[3alt] How many input files each condor job processes (default 1, matching '
                            "CRAB's Data.unitsPerJob=1).")
    parser.add_argument('--condorJobFlavour', type=str, default='workday',
                       help='[3alt] HTCondor +JobFlavour for submitted jobs (default "workday", ~1 day cap).')
    ...
    print(f"  --resubmitHeldCondorJobs: {args.resubmitHeldCondorJobs}")
    print(f"  --resubmitMissingCondorJobs: {args.resubmitMissingCondorJobs}")
    print(f"  --condorFilesPerJob: {args.condorFilesPerJob}")
    print(f"  --condorJobFlavour: {args.condorJobFlavour}")
    ...
    # datasets matching --filter for a given era as a single condor cluster --
    # condor jobs are cheap enough that per-dataset task objects aren't needed.
    # Output is written directly into the final flat preselection layout, so
    # there is no condor equivalent of --generateCrabDatasetJSON/--consolidateCrabOutput.
    if args.submitCondorJobs:
        print("\nSubmitting pre-selection jobs to HTCondor based on the DAS dataset JSON generated from --generateDASDatasetJSON...")
        submit_condor_script = base_dir / 'scripts' / 'condor' / 'submit_preselection_condor.py'
        ...
        for era in config['DASQueries']:
            ...
            dataset_json_path = output_dir / era / f"DAS_{era}_dataset.json"
            if not dataset_json_path.exists():
                print(f"Error: DAS dataset JSON file not found for era {era} at expected location: {dataset_json_path}. Please ensure --generateDASDatasetJSON has been run before submitting condor jobs.")
                continue
            golden_json_path = output_dir / era / f"{era}_goldenJSON.json"
            if not golden_json_path.exists():
                print(f"Error: Golden JSON file not found for era {era} at expected location: {golden_json_path}. Please ensure the golden JSON is downloaded before submitting condor jobs.")
                continue
            with open(dataset_json_path) as f:
                das_data = json.load(f)
            # Pre-filter to a temp JSON honoring run_all.py's own --filter semantics
            # (era/DataMC/group/dataset with wildcards, OR'd) -- submit_preselection_condor.py
            # takes a single --include/--exclude regex, which can't express that directly.
            filtered = {}
            for DataMC, groups in das_data.items():
                for group, keys in groups.items():
                    for dataset_name, files in keys.items():
                        if not matches_filter(args.filter, era, DataMC, group, dataset_name):
                            continue
                        filtered.setdefault(DataMC, {}).setdefault(group, {})[dataset_name] = files
            if not filtered:
                print(f"  No datasets match filter for era {era}, skipping.")
                continue
            filtered_json_path = output_dir / era / f"DAS_{era}_dataset_condor_filtered.json"
            with open(filtered_json_path, 'w') as f:
                json.dump(filtered, f)
            condor_output_dir = Path(storageBase) / 'preselection' / args.tag / config_hash / era
            condor_work_area = Path(storageBase) / 'condor_work' / args.tag / config_hash / era
            cmd = (
                f"python3 {submit_condor_script} --era {era} "
                f"--das-json {filtered_json_path} --golden-json {golden_json_path} "
    ...
    # Check HTCondor job status (parallel alternative to --checkCrabStatus)
    if args.checkCondorStatus:
        print("\nChecking HTCondor job status for pre-selection jobs submitted with --submitCondorJobs...")
