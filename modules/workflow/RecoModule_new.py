...
class TTbarSemilepReconstructor(Module):
    def __init__(self, era, minPgof=None):
        # Per-era muon pT thresholds matching the HLT trigger used at selection
        era_muon_pt = {
            'UL2016preVFP':  26,
            'UL2016postVFP': 26,
            'UL2017':        29,
        ...
        self.era       = era
        self.muon_pt   = era_muon_pt.get(era, 26)
        self.btag_wp   = era_btag_wp.get(era, 0.2783)
        self.min_pgof  = minPgof
        self.mW        = 80.4
        self.mt        = 172.5
        self.sigmaW    = 10.0
        self.sigmatt   = 13.0
    ...
    def analyze(self, event):
        ...
        # --- FIX: apply the same muon quality cuts used at the selection stage ---
        # Original used max(muons, ...) with no quality requirements, so any muon
        # in the event (including soft pileup muons) could be selected as the
        # "leading" muon and fed into the kinematic fit.
        selected_muons = [
            m for m in muons
            if m.pt > self.muon_pt
            and abs(m.eta) < 2.4
            and m.tightId
            and m.pfRelIso04_all <= 0.06
        ]
        if not selected_muons:
            # No tight muon — fill failure values and keep event
            chi2_status = 4
            self._fill_failure(chi2_status)
            return True
        mu    = max(selected_muons, key=lambda m: m.pt)
        mu_p4 = self.make_lep_p4(mu)

        # --- FIX: apply jet quality cuts (jetId==6 + puId) consistent with selection ---
        # Original applied only pt > 25 && |eta| < 2.4, missing the jetId and
        # PU ID requirements that were applied at the selection stage.
        selected_jets = [
            jet for jet in jets
            if jet.pt > 25
            and abs(jet.eta) < 2.4
        ...
        # Document-aligned logic:
        # 1) keep only 4 leading jets by pT
        # 2) among these 4 jets require exactly two b-tagged jets
        leading4 = sorted(selected_jets, key=lambda j: j.pt, reverse=True)[:4]
        bjets = [j for j in leading4 if j.btagDeepFlavB > self.btag_wp]
        ljets = [j for j in leading4 if j.btagDeepFlavB <= self.btag_wp]
        ...
        # Neutrino pz solutions from W mass constraint
        def nu_pz_solutions():
            px, py = met_px, met_py
        ...
        for br, bh in [(bjets[0], bjets[1]), (bjets[1], bjets[0])]:
            ...
            for pz in pz_list:
                ...
                if total_chi2 < best_chi2_prefit:
                    ...
                    best_perm = {
                        "mu_p4": mu_p4, "br_p4": br_p4, "bh_p4": bh_p4,
        ...
        if not res["success"]:
            ...
            if self.min_pgof is not None and pgof < self.min_pgof:
                chi2_status = 5
                self._fill_failure(chi2_status)
                return True
    ...
    def full_chi2_fit_soft_constraints(self, best_perm):
        ...
        def get_sigma(idx, val):
            if idx < 3:
                rel_sigma = 0.05 * abs(val)   # muon
            elif idx < 6:
                rel_sigma = 0.10 * abs(val)   # neutrino
            else:
                rel_sigma = 0.15 * abs(val)   # jets
    ...
    def make_jet_p4(self, jet):
        ...
        p4.SetPtEtaPhiM(jet.pt, jet.eta, jet.phi, 0.0)
        return p4
...
def RecoModule(era, minPgof=None):
    return TTbarSemilepReconstructor(era, minPgof=minPgof)
