...
class MuonIDWeightProducer(Module):
    ...
    def analyze(self, event):
        ...
        if leading_muon_pt > 0:  # Ensure at least one muon was selected
            ...
            if not self.warn_no_muon_once:
                print("No muons passed the selection. Default Muon ID scale factors applied.")
                self.warn_no_muon_once = True
