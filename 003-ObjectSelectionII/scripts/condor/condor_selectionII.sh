#!/bin/bash
# 003-ObjectSelectionII/scripts/condor/condor_selectionII.sh
#
# HTCondor executable for the weight/SF stage -- the condor equivalent
# of scripts/crab/crab_selectionII.sh. Every path below is passed in as an
...
#
# Args: job_id era config_yaml files_json golden_json_or_NONE is_data out_dir worker_py chapter_dir modules_dir dataset_key
...
echo "OUT_DIR=$OUT_DIR WORKER_PY=$WORKER_PY CHAPTER_DIR=$CHAPTER_DIR DATASET_KEY=$DATASET_KEY"
...
echo "Running selectionII weights..."
python3 "./$(basename "$WORKER_PY")" "$ERA" "$FILES_JSON" "$GOLDEN_JSON" "$IS_DATA" "$CHAPTER_DIR" "$DATASET_KEY"
...
if [ -z "$SKIM" ]; then
    ...
    echo "DONE condor_selectionII.sh"
    exit 0
fi
