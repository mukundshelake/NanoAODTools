...
from modules.RecoModule import RecoModule
...
def _instantiate_module(module_name, era, config):
    if module_name == "reconstruction":
        return RecoModule(era, config)
    else:
...
if __name__ == "__main__":
    ...
    logging.basicConfig(level=logging.INFO,
                        ...
                        datefmt='%Y-%m-%d %H:%M:%S')
    logging.info("Starting reconstruction processing script.")

    parser = argparse.ArgumentParser(description="Run NanoAOD reconstruction from a pre-built process list.")
