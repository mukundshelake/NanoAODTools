#!/usr/bin/env python3
"""
004A-Reconstruction/scripts/crab/submit_reconstruction_flexible.py
====================================================================
Generate and (optionally) submit CRAB jobs for the reconstruction stage,
processing the selectionII skims that already live on EOS. This is the
CPU-heavy stage (one SLSQP chi2 fit per permutation per event) and the main
motivation for CRAB support in this chapter.

Same Data.userInputFiles approach as 003-ObjectSelectionI/II's submit
scripts, for the same reason: selectionII output is not DBS-registered. See
003-ObjectSelectionI/scripts/crab/submit_selection_flexible.py's docstring
for the full rationale.

Unlike 003-I/II, no golden JSON is shipped or needed here -- reconstruction
applies neither a cut string nor golden-JSON filtering (both already handled
upstream in selectionII); see runReco.py's process_file(), which always
passes cut_string=None, jsonInput=None.

Usage
-----
    # Dry run — print what would be submitted, no actual submission
    python3 submit_reconstruction_flexible.py --era UL2018 \\
        --dataset-json inputs/selectionII_earlyApril_UL2018_datasets.json \\
        --output-lfn /store/user/mshelake/DataFiles/reconstruction/earlyApril/<hash>/UL2018 \\
        --work-area /tmp/crab_reconstruction_UL2018

    # Submit only one dataset, one file each (testing)
    python3 submit_reconstruction_flexible.py --submit --sample --era UL2018 \\
...
-------
    ...
    --dataset-json   Path to selectionII_{tag}_{era}_datasets.json (from inputs/). Required.
...
CHAPTER_DIR = SCRIPT_DIR.parent.parent           # 004A-Reconstruction/
...
def make_crab_config(era, DataMC, group, key, lfn_files, is_data, output_lfn, work_area=None):
    from WMCore.Configuration import Configuration  # noqa: PLC0415
    cfg = Configuration()
    request_name = f"reco_{safe_name(era)}_{safe_name(group)}_{safe_name(key)}"[:100]
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate/submit CRAB reconstruction jobs for a user-specified era."
    )
    ...
    parser.add_argument("--dataset-json", required=True,
                        help="Path to selectionII_{tag}_{era}_datasets.json listing the local/EOS-mounted "
                             "selectionII skim file paths.")
...
if __name__ == "__main__":
    print("Running submit_reconstruction_flexible.py")
    main()
    print("Finished submit_reconstruction_flexible.py")
