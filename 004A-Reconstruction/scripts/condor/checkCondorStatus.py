#!/usr/bin/env python3
"""
004A-Reconstruction/scripts/condor/checkCondorStatus.py
==================================================
Check status of HTCondor reconstruction jobs submitted via
submit_reconstruction_condor.py, for a given work area.
...
def read_manifest(manifest_path):
    ...
    with open(manifest_path) as f:
        for line in f:
            ...
            if not line:
                continue
            job_id, era, files_json, is_data, out_dir = line.split("\t")
            jobs.append({
                "job_id": job_id, "era": era, "files_json": files_json,
                "is_data": is_data, "out_dir": out_dir,
            })
    return jobs
...
def main():
    parser = argparse.ArgumentParser(description="Check status of condor reconstruction jobs.")
    ...
    if args.resubmitMissing and missing_jobs:
        ...
        with open(resub_manifest, "w") as f:
            for j in missing_jobs:
                f.write("\t".join([j["job_id"], j["era"], j["files_json"], j["is_data"], j["out_dir"]]) + "\n")
        submit_txt = open(submit_path).read()
        lines = [
            l if not l.startswith("queue ")
            else f"queue job_id,era,files_json,is_data,out_dir from {resub_manifest}"
