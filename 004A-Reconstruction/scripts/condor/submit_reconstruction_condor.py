#!/usr/bin/env python3
"""
004A-Reconstruction/scripts/condor/submit_reconstruction_condor.py
================================================================
Generate and (optionally) submit HTCondor jobs for the reconstruction
stage, as a parallel alternative to the CRAB path
(crab/submit_reconstruction_flexible.py).

Both paths read the same selectionII skims and write the same
{STORAGE}/reconstruction/{tag}/{hash}/{era}/{DataMC}/{group}/{dataset}/ layout,
...
-----
    ...
    python3 004A-Reconstruction/scripts/condor/submit_reconstruction_condor.py \\
...
CHAPTER_DIR = SCRIPT_DIR.parent.parent          # condor/ -> scripts/ -> 004A-Reconstruction/
CONFIG_YAML = CHAPTER_DIR / "config.yaml"
MODULES_DIR = SCRIPT_DIR.parent / "modules"
WRAPPER_SH = SCRIPT_DIR / "condor_reconstruction.sh"
WORKER_PY = SCRIPT_DIR / "condor_script_reconstruction.py"
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate/submit HTCondor reconstruction jobs for one era."
    )
    ...
    parser.add_argument("--output-dir", required=True,
                        help="Selection output base for this era, e.g. "
                             "{STORAGE}/reconstruction/{tag}/{hash}/{era}. Must be on EOS.")
    ...
    for path, label in [
        (WRAPPER_SH, "condor_reconstruction.sh"),
        (WORKER_PY, "condor_script_reconstruction.py"),
        ...
        for mod_name in config["ModuleList"].get(data_mc_key, []):
            if mod_name not in MODULE_FILES:
                print(f"ERROR: ModuleList.{data_mc_key} names '{mod_name}', which the condor "
                      ...
                      f"condor_script_reconstruction.py's build_modules().", file=sys.stderr)
                sys.exit(1)

    # RecoModule needs only the era string -- no correction files to check here,
    # unlike 003-ObjectSelectionI/II.
    ...
    submit_path = work_area / "reconstruction.sub"
    ...
    for job_id, file_chunk, is_data, dataset_out_dir in job_specs:
        ...
        manifest_lines.append("\t".join([
            job_id, args.era, str(files_json_path),
...
arguments = "$(job_id) $(era) {CONFIG_YAML} $(files_json) $(is_data) $(out_dir) {WORKER_PY} {CHAPTER_DIR} {MODULES_DIR}"
...
queue job_id,era,files_json,is_data,out_dir from {manifest_path}
"""
...
if __name__ == "__main__":
    print("Running submit_reconstruction_condor.py")
    main()
    print("Finished submit_reconstruction_condor.py")
