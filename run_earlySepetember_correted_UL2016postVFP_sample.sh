...
# run for era UL2016postVFP (Data_mu + MC_mu), tag "earlySepetember_correted",
# full ABCD treatment, exercising every correction added this session:
# additional-lepton veto (003-I SelectionCuts), METXYCorr, jetVetoMap, JetJER,
# MuonRochester (all via PreSelectionCorrectionModuleList/ModuleList), and
# TopPtWeight (003-II).
#
...
#
# Usage: bash run_earlySepetember_correted_UL2016postVFP_sample.sh
...
notify "Starting sample run (full ABCD, all corrections: veto/JER/METXYCorr/jetVetoMap/MuonRochester/TopPtWeight)."
...
log "Running pre-selection corrections pass (jetVetoMap/jetJER/muonRochester, --sample)..."
python3 scripts/run_all.py --tag "$TAG" --applyPreSelectionCorrections --sample \
...
log "Reusing existing UL2016postVFP b-tagging/jetPUID efficiency maps (not recomputed for this sample run)."
...
log "Running 003-II SF-weighted skim production (--sample; includes topPtWeight)..."
...
log "=== DONE (sample run). Report: $REPORT_PDF ==="
