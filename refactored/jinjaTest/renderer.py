...
def processMuonIDWeightConfigs(data, env, era):
    ...
    outputDir = data.get("tag", "outputs")
    os.makedirs(outputDir, exist_ok=True)
    with open(os.path.join(outputDir, f"muonIDWeight_{era}_config.yaml"), "w") as f:
        f.write(renderedMuonIDWeight)
    print(f"Rendered {outputDir}/muonIDWeight_{era}_config.yaml")
...
def processMuonHLTWeightConfigs(data, env, era):
    ...
    muonHLTWeightData = {
        ...
        "muon_eta_lo": data["muon"]["eta"]["lo"][era],
        "muon_eta_hi": data["muon"]["eta"]["hi"][era],
        "weightName": data["HLTSFParams"]["weightName"],
    ...
    outputDir = data.get("tag", "outputs")
    os.makedirs(outputDir, exist_ok=True)
    with open(os.path.join(outputDir, f"muonHLTWeight_{era}_config.yaml"), "w") as f:
        f.write(renderedMuonHLTWeight)
    print(f"Rendered {outputDir}/muonHLTWeight_{era}_config.yaml")
