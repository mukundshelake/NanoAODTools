#!/usr/bin/env python3
"""
003-ObjectSelectionI/scripts/condor/checkCondorStatus.py
==================================================
Check status of HTCondor object-selection jobs submitted via
submit_selection_condor.py, for a given work area.
...
def read_manifest(manifest_path):
    ...
    with open(manifest_path) as f:
        for line in f:
            ...
            if not line:
                continue
            job_id, era, files_json, golden_json, is_data, out_dir = line.split("\t")
            jobs.append({
...
def main():
    parser = argparse.ArgumentParser(description="Check status of condor object-selection jobs.")
    ...
    if args.resubmitMissing and missing_jobs:
        ...
        with open(resub_manifest, "w") as f:
            for j in missing_jobs:
                f.write("\t".join([j["job_id"], j["era"], j["files_json"], j["golden_json"], j["is_data"], j["out_dir"]]) + "\n")
        submit_txt = open(submit_path).read()
        lines = [
            l if not l.startswith("queue ")
            else f"queue job_id,era,files_json,golden_json,is_data,out_dir from {resub_manifest}"
