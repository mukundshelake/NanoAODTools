...
def load_module(module_name, era, key=None, config=None):
    ...
    if module_name == "lheWeightSign":
        ...
        loaded = muonIDWeightModule(config["moduleConfigs"]["MuonIDWeight"])
    elif module_name == "muonHLT":
        from python.postprocessing.modules.custom.MuonHLTWeight import muonHLTWeightModule
        loaded = muonHLTWeightModule(config["moduleConfigs"]["MuonHLTWeight"])
...
def process_file(data):
    stage, era, DataMC, key, outputDir, file, conf = data
    files = [file]
    ...
    if "Data" in DataMC:
        if conf["modules"]["Data"] is not None:
            ...
            for mod in conf["modules"]["Data"]:
                modules.append(lambda m=mod: load_module(m, era, key, conf))
    elif "MC" in DataMC:
                        ...
                        if is_root_file_healthy(file):
                            ...
                            if os.path.exists(destFile):
                                if is_root_file_healthy(destFile):
                                    ...
                                    continue
                            process_list.append((stage, era, DataMC, key, outputDir, file, config[stage][era]))
                            if runSample:
