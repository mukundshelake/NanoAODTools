#!/usr/bin/env python3
"""
getObservables_reco_woutBDT.py

Compute reco-level observables for MC qqbar events (y == 1), without any BDT cut.
...
Usage:
    python getObservables_reco_woutBDT.py \\
        --era UL2016preVFP \\
...
Output:
    ...
    The "reco" histogram has a leading "systematic" StrCategory axis:
        no --syst:  ["nominal"]  (unweighted event counts for qqbar only)
        --syst:     ["nominal",
                     "muonIDUp",         "muonIDDown",
...
class RecoWoutBDTObservablesProcessor(processor.ProcessorABC):
    """
    Coffea ProcessorABC for reco-level qqbar-only ttbar charge-asymmetry observables.

    Selects events with y == 1 (qqbar initial state) — no BDT cut applied.
    ...
    def __init__(self, era: str, output_folder: str, syst: bool = False):
        ...
        # qqbar filter (y == 1)
        y_mask = ak.to_numpy(events["y"] == 1)
        events = events[y_mask]
        idx    = idx[y_mask]
...
def main():
    parser = argparse.ArgumentParser(
        description="Reco-level qqbar-only observables (no BDT cut)."
    )
    ...
    print(f"Era: {args.era}  |  Muon pT threshold: {muon_pt} GeV  |  qqbar only (y==1)")
    if args.syst:
    ...
    out, metrics = run(
        fileset,
        processor_instance=RecoWoutBDTObservablesProcessor(
            era=args.era,
