...
Usage:
    python getObservables_reco_woutBDT.py \\
        ...
        --output_name reco_qqbar_UL2016preVFP
...
Output:
    ...
    The coffea file stores:
        results[group][dataset] = {
            ...
            "nEvents": int        – events passing y==1 + muon selection
...
BRANCHES = [
    ...
    "y",        # parton type: 1=qqbar, 2=gg, 3=qg, 4=qqprime, 5=qq, 0=undefined
]
...
def process_dataset(
    ...
    """
    Apply y==1 filter + muon selection, compute reco observables, fill histogram.
...
def main():
    parser = argparse.ArgumentParser(
        description="Reco-level observables for MC qqbar events (no BDT cut)."
    )
    ...
    print("Mode: qqbar-only (y==1), no BDT cut")
    ...
    for group, group_datasets in datasets.items():
        ...
        for dataset_name, file_dict in group_datasets.items():
            ...
            h, n_events = process_dataset(data, muon_pt_thresh)
