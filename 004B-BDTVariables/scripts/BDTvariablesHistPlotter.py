#!/usr/bin/env python3
"""
Create CMS-style Data/MC stacked comparison plots for BDT variables from
aggregated .coffea histograms.

Usage:
    python scripts/BDTvariablesHistPlotter.py \\
        --tag earlyApril \\
        --hash 0e454e8b0284 \\
...
Outputs per era (inside --outputDir/{era}/):
  - {histName}.png
  - {histName}.pdf
  - BDTvariablesHists.root  (all TH1F objects)
"""
...
def get_aggregated_coffea_path(output_dir: Path, tag: str, era: str,
    ...
    return output_dir / era / data_mc / group / f"{tag}_{era}_{data_mc}_{group}_bdtvariableHists.coffea"
...
def process_era(era: str, config: dict, output_dir: Path, tag: str, args):
    ...
    root_file = TFile(str(era_out / "BDTvariablesHists.root"), "RECREATE")
    ...
    def stack_order(g):
        return mc_mu_plot.get(g, {}).get('order', 99)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Create CMS-style Data/MC ROOT histograms for BDT variables from aggregated coffea files.")
