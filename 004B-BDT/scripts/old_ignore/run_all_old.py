#!/usr/bin/env python3
"""
Master orchestration script for 004B-BDT Data/MC plotting workflow

Supports BDT variables and BDT parton-level analysis types.
Follows the 002-Samples pattern with hash-based output directories for reproducibility.

Usage:
    # Basic usage (reads from config.yaml, runs both BDT analyses)
    python scripts/run_all.py

    # Run only BDT variables analysis
    python scripts/run_all.py --analysis-type bdtvariables

    # Run only BDT parton-level analysis
    python scripts/run_all.py --analysis-type bdtvariables_parton

    # Override config settings
    python scripts/run_all.py --eras UL2017 --tag midNov

    # Plot specific variables only
    python scripts/run_all.py --variables var1,var2 --analysis-type bdtvariables
...
def run_command(cmd, description):
    ...
    # Run from 004B-BDT directory (parent of scripts)
...
def main():
    parser = argparse.ArgumentParser(
        description="Master script to generate BDT variables Data/MC plots",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter
    )

    # Analysis type selection
    parser.add_argument('--analysis-type', type=str,
                        choices=['bdtvariables', 'bdtvariables_parton', 'all'],
                        default='all',
                        help='Type of analysis to run: bdtvariables, bdtvariables_parton, or all')
    ...
    if args.analysis_type == 'all':
        analysis_types = ['bdtvariables', 'bdtvariables_parton']
    else:
    ...
    for analysis_type in analysis_types:
        ...
        for era in eras:
            ...
            process_era(era, tag, analysis_type, variables, config_path, args)
...
def process_era(era, tag, analysis_type, variables, config_path, args):
    """
    Process a single era for a given BDT analysis type.

    Args:
        era:           Era name (e.g., 'UL2017')
        tag:           Tag identifier (e.g., 'midNov')
        analysis_type: 'bdtvariables' or 'bdtvariables_parton'
    ...
    # Compute config hash (files have distinguishing names within the same hash dir)
    ...
    if not args.skip_histograms:
        cmd = [
            sys.executable,
            f"scripts/{hist_script}",  # Path relative to 004B-BDT
            '-e', era,
            '-t', tag,
        ]
    ...
    if not args.skip_plots:
        ...
        cmd = [
            sys.executable,
            f"scripts/{plotter_script}",  # Path relative to 004B-BDT
            str(coffea_file),
