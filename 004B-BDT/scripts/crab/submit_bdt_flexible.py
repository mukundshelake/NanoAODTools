...
Same Data.userInputFiles approach as 003-ObjectSelectionI/II and
004A-Reconstruction's submit scripts, for the same reason: reconstruction
output is not DBS-registered. See 003-ObjectSelectionI's
submit_selection_flexible.py docstring for the full rationale.

No golden JSON is shipped or needed -- runBDTVariables.py always passes
cut_string=None, jsonInput=None (both already handled upstream). No module
config is needed either -- BDTvariableModule() takes no parameters.

Usage
-----
    # Dry run — print what would be submitted, no actual submission
    python3 submit_bdt_flexible.py --era UL2018 \\
        --dataset-json inputs/reconstruction_earlyApril_UL2018_datasets.json \\
        --output-lfn /store/user/mshelake/DataFiles/BDTVariables/earlyApril/<hash>/UL2018 \\
        --work-area /tmp/crab_bdt_UL2018

    # Submit only one dataset, one file each (testing)
    python3 submit_bdt_flexible.py --submit --sample --era UL2018 \\
...
-------
    ...
    --dataset-json   Path to reconstruction_{tag}_{era}_datasets.json (from inputs/). Required.
...
SCRIPT_PY   = SCRIPT_DIR / "crab_script_bdt.py"
MODULE_PY   = CHAPTER_DIR / "scripts" / "modules" / "BDTvariableModule.py"
...
def make_crab_config(era, DataMC, group, key, lfn_files, is_data, output_lfn, work_area=None):
    ...
    cfg = Configuration()
    request_name = f"bdt_{safe_name(era)}_{safe_name(group)}_{safe_name(key)}"[:100]
    ...
    cfg.JobType.scriptArgs = [f"isData={is_data}"]
    cfg.section_("Data")
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate/submit CRAB BDT-variable jobs for a user-specified era."
    )
    ...
    parser.add_argument("--dataset-json", required=True,
                        help="Path to reconstruction_{tag}_{era}_datasets.json listing the local/EOS-mounted "
                             "reconstruction skim file paths.")
    ...
    for path, label in [
        (SCRIPT_SH,   "crab_bdt.sh"),
        (SCRIPT_PY,   "crab_script_bdt.py"),
        (MODULE_PY,   "BDTvariableModule.py"),
        (PSET,        "PSet.py"),
...
if __name__ == "__main__":
    print("Running submit_bdt_flexible.py")
    main()
    print("Finished submit_bdt_flexible.py")
